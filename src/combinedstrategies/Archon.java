...
strictfp class Archon extends RobotPlayer implements RobotHandler {
	...
	// TODO(daniel): how do different values affect the outcome? I suspect not
	// by much.
	...
	public void init() throws GameActionException {
		if (rc.getRoundNum() == 1) {
			...
			// the first turn is a heartbeat turn, so unit counts aren't
			// updated.
			// force a manual update (even though some information may only be
			// partially correct)
	...
	public void onLoop() throws GameActionException {
		...
		if (rc.getRoundNum() <= 2) {
			if (rc.getRoundNum() == 1) {
				...
				int buildOrder;
				// TODO: determine these coefficients by doing a regression over
				// win rates
				...
				// on the first round of construction, different archons may
				// have sensed different things.
				// If someone saw a tree with a gift, only build a lumberjack if
				// you are that archon.
				// If it's a good idea to rush, only build a soldier if you're
				// that archon.
				// If that's impossible (eg DigMeOut), don't do anything. Next
				// turn another archon can pick up the slack.
				...
				if (buildOrder == BuildOrder.LUMBERJACK_FIRST && anyTreesWithGifts) {
					...
					shouldBuildFirst = true;
				} else if (buildOrder == BuildOrder.ECONOMIC || (buildOrder == BuildOrder.LUMBERJACK_FIRST
						&& treeHealthNearby > AVG_TREE_HEALTH_THRESHOLD && !Messaging.getHasGift())) {
					// it's economic, or
					// it's lumberjacks, and there's lots of trees near us (but
					// no gifts)
					shouldBuildFirst = true;
				}
		...
		if (Messaging.gardenerCount <= 15 && (Messaging.gardenerCount < 4 || Messaging.soldierCount
				+ Messaging.lumberjackCount + Messaging.tankCount > 3 * Messaging.gardenerCount)) {
			builtGardener = tryBuildGardener();
		}
		if (!tryKiteLumberJacks(9000)) {
			if (!tryDodgeBullets(10000)) {
				if (needToMoveAwayFromPack()) {
					// maybe we should always try to stay on the exterior of the
					// convex hull of trees? otherwise we could
	...
	boolean tryBuildGardener() throws GameActionException {
		...
		// we could also check for bullets, but we'd have to look 20 turns
		// ahead.
	...
	static boolean waitingForUndefendedGardener() {
		...
		// so if there's already a gardener and no military, don't build more
		// gardeners
		...
		if (shouldWait && rc.getTeamBullets() >= 100f) {
			...
			// the new gardener hasn't built anything. maybe he's
			// stuck/obstructed/throwing exceptions?
	...
	// Gardeners get less efficient over time, due to crowding. Down-weight
	...
	boolean moreEfficientToDoNothing() {
		// if we're close to winning, it might be more efficient to not build
		// anything, since trees take a while to
		// mature.
		// Due to map layouts and enemy activity, it's difficult to compute
		// exactly how much money per turn a
		// gardener will eventually yield, or how long he will take to finish
		// all his trees.
		// However, in the optimal case (for both 4 and 5 trees), it takes 171
		// turns to pay for a gardener and his
		// trees.
		// instead, just estimate the #trees/gardener based on the reported
		// income.
